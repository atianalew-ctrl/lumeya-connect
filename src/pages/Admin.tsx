...
import { Plus, Trash2, Edit2, Save, X, Upload, Loader2, Users, Star, Instagram, ImageIcon } from "lucide-react";
...
const CreatorForm = ({ initial, onSave, onCancel }: {
  ...
  const handlePhoto = (file: File) => {
    ...
    reader.onload = () => { set("avatar_url", reader.result as string); setUploading(false); toast.success("Photo ready!"); };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="flex items-start gap-4">
        ...
        >
          {form.avatar_url
            ...
            : uploading
              ...
              : <div className="text-center p-2"><ImageIcon size={18} className="mx-auto text-muted-foreground mb-1" /><p className="text-[10px] text-muted-foreground">Upload photo</p></div>
          }
        ...
        <div className="flex-1 space-y-3">
          <div>
            ...
            <Input required placeholder="e.g. Ronja Aaslund" value={form.display_name} onChange={e => set("display_name", e.target.value)} />
          </div>
          <div>
      ...
      <div>
        ...
        <Input type="number" step="0.1" min="0" max="5" placeholder="4.9" value={form.rating || ""} onChange={e => set("rating", Number(e.target.value))} className="w-32" />
      </div>
...
const Admin = () => {
  ...
  const fetch = async () => {
    setLoading(true);
    const { data } = await supabase.from("creator_profiles").select("id,display_name,tagline,location,bio,instagram,rates,tags,avatar_url,rating,created_at").order("created_at", { ascending: false });
  ...
  const handleAdd = async (data: Omit<Creator, "id" | "created_at">) => {
    const { error } = await supabase.from("creator_profiles").insert({ display_name: data.display_name, tagline: data.tagline, location: data.location, bio: data.bio, instagram: data.instagram, rates: data.rates, tags: data.tags, avatar_url: data.avatar_url, rating: data.rating });
  ...
  const handleEdit = async (data: Omit<Creator, "id" | "created_at">) => {
    if (!editingId) return;
    const { error } = await supabase.from("creator_profiles").update({ display_name: data.display_name, tagline: data.tagline, location: data.location, bio: data.bio, instagram: data.instagram, rates: data.rates, tags: data.tags, avatar_url: data.avatar_url, rating: data.rating }).eq("id", editingId);
    if (error) { toast.error("Failed: " + error.message); return; }
  ...
  const startEdit = (c: Creator) => {
    setEditingId(c.id);
    setEditInitial({ display_name: c.display_name, tagline: c.tagline, location: c.location, bio: c.bio, instagram: c.instagram, rates: c.rates, tags: c.tags, avatar_url: c.avatar_url, rating: c.rating });
    setShowForm(false);
  };
    ...
    <div className="container max-w-3xl py-16 px-6">
      <div className="flex items-end justify-between mb-10">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Add real creators — they'll appear live on the site</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 border border-dashed border-border rounded-2xl">
          ...
          <p className="text-muted-foreground text-xs mt-1">Click "Add Creator" to get started.</p>
        </div>
      ) : (
